    ...
    try {
        ...
        const query = `
            ...
            AND NOT EXISTS (
                ...
                AND upq.user_id = ?
                AND upq.game_type = 'jumble_words'
        ...
        const question = Array.isArray(results) ? results[0] : results; // Fetch the question from the results
...
exports.markQuestionAsPlayed = async (req, res) => {
    ...
    try {
        ...
        const checkQuestionQuery = "SELECT id, answer,level,day FROM jumbled_words WHERE id = ?";
...
const deductJumbleLifeIfNotPlayed = async (req) => {
    ...
    try {
      ...
      const [playedData] = await executeQuery(
        ...
        `,
        [userId, questionId, "jumble_words", program_type]
      );
      ...
      if (!playedData) {
        ...
        if (user && user.jumble_life > 0) {
          // Step 3: Deduct 1 from jumble_life
          await executeQuery(
            `
            UPDATE users_data
            SET jumble_life = jumble_life - 1
            WHERE id = ?;
            `,
            [userId]
          );
          ...
          return {
            success: true,
            message: "Jumble life deducted and question logged successfully.",
          };
        } else {
          return {
            success: false,
            message: "Insufficient jumble life.",
          };
        }
      } else {
        return{
...
exports.getUserLife=async(req,res)=>{
    try {
       ...
       const data= await executeQuery('SELECT jumble_life as life FROM users_data WHERE id = ?',[userId])
...
exports.getgameslife=async(req,res)=>{
    try {
        const {userId}=req.query;
       const data= await executeQuery('SELECT jumble_life , memory_life, perfectmatch_life,missing_life  FROM users_data WHERE id = ?',[userId])
