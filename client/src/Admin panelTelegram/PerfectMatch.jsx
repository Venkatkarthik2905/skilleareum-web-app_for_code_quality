...
export default function PerfectMatch() {
    ...
    const fetchMissingWords = async () => {
        try {
            const response = await axios.get(`${SERVER_URL}/api/getPerfectMatch`);
    ...
    const handleEditClick = (index) => {
        ...
        setEditedData({
            ...
            answer: (entity.answer || "").toUpperCase(),
    ...
    const handleSaveClick = async (id) => {
        try {
            const response = await axios.post(`${SERVER_URL}/api/updatePerfectMatch`, {
                id,
                ...editedData,
                answer: editedData.answer.toUpperCase()
            });
    ...
    const handleDeleteClick = async (id) => {
        try {
            const response = await axios.delete(`${SERVER_URL}/api/deletePerfectMatch`, {
                data: { id }
            });
            if (response.data.success) {
    ...
    const handleAddClick = async () => {
        try {
            const response = await axios.post(`${SERVER_URL}/api/addPerfectMatch`,  {
                ...editedData,
                answer: editedData.answer.toUpperCase() 
            });

            if (response.data.success) {
                alert(' Missing Word added successfully');
                // setTableEntries([...tableEntries, response.data.newEntry]); 
                fetchMissingWords();
            } else {
                alert('Failed to add  Missing Word');
            }
        } catch (error) {
            console.error('Error adding  Missing Word:', error);
            alert('Error adding  Missing Word');
        }
    };
    ...
    const handleChange = (e) => {
        ...
        setEditedData((prevData) => ({
            ...prevData,
            [name]: name === "answer" ? value.toUpperCase() : value
        }));
    };

    return (
        <>
            <div className="font-san mt-2 bg-white/5 rounded-2xl py-10 px-5 md:p-5">
                <p className="text-[#38B6FF] flex items-center gap-2 font-bold">Perfect Match Vault</p>
                            ...
                            <tbody className="divide-y divide-white/5">
                                {currentEntities.map((entity, index) => (
                                    <tr key={index} className="h-16  text-xs md:text-sm cursor-pointer rounded-xl">
                                        ...
                                        <td className="px-5 lg:px-3 py-2 whitespace-normal break-all">
                                            {editingIndex === index ? (
                                                <input
                                                    type="text"
                                                    name="hintImg"
                                        ...
                                        <td className="px-5 lg:px-3 py-2 whitespace-normal ">
                                            {editingIndex === index ? (
                                                <input
                                                    type="text"
                                                    name="answer"
                                        ...
                                        <td className="flex flex-col justify-center items-center gap-2 h-full mt-10">
                                            {editingIndex === index ? (
                                                <>
                                                    <button
                                                        className="bg-blue-500 font-semibold rounded-md w-full px-3 py-1"
                                                        onClick={() => handleSaveClick(entity.id)}
                                                    >
                                                        Save
                                                    </button>
                                                    <button
                                                        className="bg-red-500 font-semibold rounded-md w-full px-3 py-1"
